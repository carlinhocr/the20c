...
DATA_IN     = %00001000         ; PB3 - DATA input (non-inverting: 0=bus low/asserted, 1=bus high)
CLK_IN      = %00010000         ; PB4 - CLK  input (non-inverting: 0=bus low/asserted, 1=bus high)
...
;          Carry flag = 0 if DATA is HIGH (released)
;
; WIRING NOTE: this board's input path is NON-INVERTING - the VIA reads
; the bus level directly:
;     bit = 0  ->  bus LOW   (asserted)
;     bit = 1  ->  bus HIGH  (released)
; The carry-out contract is unchanged (carry set = asserted); only the
; bit->carry mapping is flipped, so no caller changes.
;-----------------------------------------------------------------------
READ_DATA:
            LDA VIA_PORTB       ; Read port
            AND #DATA_IN        ; Isolate DATA input bit
            BEQ .data_low       ; bit 0 = bus LOW = asserted
            CLC                 ; bit 1 = bus HIGH (released) -> clear carry
            RTS
.data_low:
            SEC                 ; bus LOW (asserted) -> set carry
            RTS
...
READ_CLK:
            LDA VIA_PORTB       ; Read port
            AND #CLK_IN         ; Isolate CLK input bit
            BEQ .clk_low        ; bit 0 = bus LOW = asserted
            CLC                 ; bit 1 = bus HIGH (released) -> clear carry
            RTS
.clk_low:
            SEC                 ; bus LOW (asserted) -> set carry
