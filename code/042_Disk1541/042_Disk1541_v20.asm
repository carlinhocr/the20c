...
;     v17->v18  IEC_SEND_BYTE Y-register corruption fixed (save/restore Y).
;     v18->v19  ZP_TEMP reuse fixed; dedicated ZP_CHAN added; clobber audit.
;     v19->v20  WRITE to a REAL 1541/1571 fixed. The "ready-for-data" wait in
;               IEC_SEND_BYTE_ATN / IEC_SEND_BYTE timed out in ~492 ms, but a
;               real drive holds DATA low ~1 s while it creates the file after
;               OPEN. So the WRITE data phase (LISTEN + DATA-secondary) timed
;               out and the stream desynced. SD2IEC creates files instantly
;               and never hit it; format never needs a command phase during a
;               busy window. Fix: ms-based ~2 s RFD timeout (fast path
;               unchanged) + status checks after the data-phase LISTEN/
;               secondary so a real failure aborts cleanly. See RFD_TIMEOUT_CHUNKS.
;
;===============================================================================
;
;   HARDWARE SETUP:
...
; Readiness (RFD) timeout for the byte handshake, in ~256 ms chunks.
; A REAL 1541/1571 pulls DATA low and holds it while it is busy creating a
; file after OPEN-for-write (directory search + block allocation). The v19
; capture showed this taking ~1 s. The listener may legitimately hold off
; this long, so the "ready-for-data" wait must exceed it. 8 -> ~2 s.
; (This is the v20 fix; see the note in IEC_SEND_BYTE_ATN.)
RFD_TIMEOUT_CHUNKS = 8
...
; --- Wait for Ready For Data: listener releases DATA (HIGH) ---
            ; *** v20 FIX (write-to-real-1571 failure) ***
            ; A real 1541/1571 holds DATA LOW while it is BUSY creating the
            ; file right after OPEN-for-write (directory search + first-block
            ; allocation). The v19 trace (Session_23) showed the drive holding
            ; DATA low for ~984 ms here. The old tight X/Y poll timed out in
            ; ~492 ms, so the first two command bytes of the WRITE data phase
            ; (LISTEN $28, then DATA-secondary $61) BOTH timed out - the two
            ; 492 ms flat waits visible in the capture - and the byte stream
            ; then ran into a desynced bus. An SD2IEC creates the file almost
            ; instantly, so it always answered inside 492 ms and never tripped
            ; this - which is exactly why the bug only appeared on real drives.
            ; Format also dodged it: its slow work happens AFTER close, with no
            ; second command phase to land during a busy window.
            ;
            ; Fix: a generous, millisecond-based readiness timeout (~2 s). The
            ; FAST PATH is unchanged - READ_DATA is tested first each pass, so
            ; when the drive is already ready we exit before any DELAY_1MS and
            ; per-byte streaming throughput is unaffected. Only a genuinely
            ; busy drive spends real time here.
            LDX #RFD_TIMEOUT_CHUNKS  ; outer: ~256 ms per chunk
.atn_rfd_outer:
            LDY #$00                 ; inner: 256 x ~1 ms
.atn_wait_rfd:
            JSR READ_DATA
...
; --- Wait for Ready For Data: listener releases DATA (HIGH) ---
            ; *** v20 FIX *** Same generous timeout as IEC_SEND_BYTE_ATN: the
            ; listener can hold DATA low for up to ~1 s (file creation, or a
            ; buffer flush between blocks). Fast path exits immediately when
            ; the drive is ready, so streaming speed is unchanged. Y is used
            ; as the inner counter but is restored from the stack on every
            ; exit path (TYA/PHA at entry, PLA/TAY below), so the caller's
            ; loop index survives. DELAY_1MS preserves X, so the outer chunk
...
.send_wait_ready:
            JSR READ_DATA
            BCC .listener_ready      ; DATA HIGH (released) = listener ready!
            JSR DELAY_1MS
            DEY
            BNE .send_wait_ready
            DEX
...
.write_opened_ok:
            ...
            ; *** v20 *** Before v20 the result of this LISTEN (and the
            ; DATA-secondary below) was never checked, so a timed-out setup
            ; fell straight through into the data loop and streamed 256 bytes
            ; onto a bus the drive was not listening to. Check it now.
            LDA IEC_STATUS
            BEQ .write_listen_ok
            JMP .write_error_cleanup
.write_listen_ok:
