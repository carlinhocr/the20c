...
EOI_WINDOW     = 200            ; T2 count = ~200 us at 1 MHz. THE protocol
...
;-----------------------------------------------------------------------
; IEC_WAIT_RFD - Wait for the listener to release DATA (ready-for-data)
;
; This is THE long wait in the driver. Called once per byte, immediately
; after the talker has released CLK (RTS). The listener answers by
; releasing DATA. On a real drive that answer can be over a second when
; the byte in question is the one that follows an OPEN-for-write, because
; the drive is searching the directory and allocating a block.
;
; Output: Carry CLEAR = listener released DATA, ready to receive
;         Carry SET   = timed out after RFD_TIMEOUT_MS
;
; Clobbers: A, ZP_RFDCNT, VIA Timer 2. Preserves X and Y.
;
; Timing:   T2 is armed for RFD_CHUNK_MS at a time and the chunks are
;           counted, so the budget is in real milliseconds and does not
;           depend on the cycle cost of this loop. DATA is still polled
;           every ~30 cycles, so a fast listener (SD2IEC answers in tens
;           of microseconds) is not slowed down by the coarse budget.
;
; Requires: ACR bit 5 = 0 (T2 timed-interrupt mode). RESET writes ACR=$00.
;           Timer 1 is untouched - it belongs to DELAY_US and the bit
;           framing, and the two never overlap.
;-----------------------------------------------------------------------
IEC_WAIT_RFD:
            LDA #RFD_OUTER
            STA ZP_RFDCNT
.rfd_arm:
            LDA #<RFD_CHUNK
            STA VIA_T2CL        ; low byte of the count
            LDA #>RFD_CHUNK
            STA VIA_T2CH        ; writing T2CH clears T2's flag and starts it
.rfd_poll:
            JSR READ_DATA
            BCC .rfd_ready      ; DATA HIGH (released) = listener ready
            LDA VIA_IFR
            AND #%00100000      ; bit 5 = Timer 2 flag
            BEQ .rfd_poll       ; chunk still running, keep polling DATA
            DEC ZP_RFDCNT
            BNE .rfd_arm        ; more chunks left in the budget
            SEC                 ; budget exhausted
            RTS
.rfd_ready:
            CLC
            RTS
...
;-----------------------------------------------------------------------
; IEC_WAIT_CLK_RTS - Wait for the talker to release CLK (RTS, CLK HIGH)
;
; The receive-side mirror of IEC_WAIT_RFD, and just as slow-tolerant: the
; drive may be reading a sector before it can offer the next byte.
;
; Output: Carry CLEAR = CLK released, talker ready to send
;         Carry SET   = timed out after RTS_TIMEOUT_MS
; Clobbers: A, ZP_RFDCNT, VIA Timer 2. Preserves X and Y.
;-----------------------------------------------------------------------
IEC_WAIT_CLK_RTS:
            LDA #RTS_OUTER
...
.rts_poll:
            JSR READ_CLK
            BCC .rts_ready      ; CLK HIGH (released) = talker ready to send
            LDA VIA_IFR
            AND #%00100000      ; bit 5 = Timer 2 flag
            BEQ .rts_poll
            DEC ZP_RFDCNT
...
;-----------------------------------------------------------------------
; IEC_WAIT_CLK_LOW - Wait for the talker to pull CLK LOW
;
; Used only after we have acknowledged EOI: the talker then pulls CLK low
; and clocks the final byte out normally.
;
; Output: Carry CLEAR = CLK asserted
;         Carry SET   = timed out after CLKLOW_TIMEOUT_MS
; Clobbers: A, ZP_RFDCNT, VIA Timer 2. Preserves X and Y.
;-----------------------------------------------------------------------
IEC_WAIT_CLK_LOW:
            LDA #CLKLOW_OUTER
...
.cl_poll:
            JSR READ_CLK
            BCS .cl_low         ; CLK LOW = talker is starting the byte
            LDA VIA_IFR
            AND #%00100000
            BEQ .cl_poll
            DEC ZP_RFDCNT
...
;
; Call immediately after releasing DATA (RFD). The talker's answer to RFD
; is to pull CLK LOW and start clocking. If it does not do that within
; ~200 us, it is deliberately stalling to say "last byte" - that stall IS
; the EOI signal. There is no separate EOI line and no length field.
;
...
;
; The poll is ~31 cycles, giving ~6 samples across the window. The CLK-low
; phase of a real drive's bit is tens of microseconds, so it cannot slip
; between samples.
;-----------------------------------------------------------------------
IEC_EOI_WINDOW:
            LDA #<EOI_WINDOW
...
.eoi_poll:
            JSR READ_CLK
            BCS .eoi_normal     ; CLK LOW = talker started the byte in time
            LDA VIA_IFR
            AND #%00100000
            BEQ .eoi_poll
