...
void onClock (){
  ...
  sprintf(output, " %04x %01c %02x ",address,digitalRead(READ_WRITE)?'r':'W',data,chip);
  Serial.print(output);
  Serial.println(chip);
}
...
char *find_chip(unsigned int prefix_address) {
  if (prefix_address >= 4096){ //8000 hex
    return "ROM  ";
   } else if (prefix_address >= 256 and prefix_address < 512){ //$0000 y $4000
    return "STACK ";   
  } else if (prefix_address >= 0 and prefix_address < 4095){ //$0000 y $4000
    return "RAM ";
