...
A few things to keep in mind for your hardware build: the VIA PB0–PB2 outputs need open-collector drivers (transistors or buffers with external pull-up resistors to +5V, typically 1kΩ) since the IEC bus is active-low. The input pins PB3–PB4 should read the actual bus state after the pull-ups. The .byte/.word/.org directives follow common 6502 assembler syntax (ca65, DASM, etc.) but you may need minor adjustments for your specific assembler.