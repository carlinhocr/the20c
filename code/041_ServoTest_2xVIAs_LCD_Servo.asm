...
RESET:
  ...
  ;set all port B pins as output
  lda #%11111111  ;load all ones equivalent to $FF
  sta SERVO_DDRB ;store the accumulator in the data direction register for Port B

  ;set all port A pins as output
  lda #%11111111  ;load all ones equivalent to $FF
  sta SERVO_DDRA ;store the accumulator in the data direction register for Port A
  ;END Configure Ports A & B
...
wait_18ms_wait_2ms_loop:
  dex ;2 cycles
  beq wait_18ms_inner 
  jmp wait_18ms_wait_2ms_jump_to_wait ;3cycles
wait_18ms_wait_2ms_jump_to_wait:  
  jmp wait_18ms_wait_2ms_loop ;3cycles
wait_18ms_end:
  rts
...
wait_0_5ms:
  ...
  ;500 cycles are 50 runnings of the loop = 0.5 millisecond
  ;adding the rts that is 6 cycles ~ 0.5 millisecond
wait_0_5ms_loop:
  dex ;2 cycles
  beq wait_0_5ms_loop_end ;2cycles to evaluate + 1 or to 2 to take branch
  jmp wait_0_5ms_jump_to_wait ;3cycles
wait_0_5ms_jump_to_wait:  
  jmp wait_1ms_loop ;3cycles
wait_0_5ms_loop_end:
  rts  ;6 cycles
...
;   ldx #200
;   ;10 cycles total per loop
;   ;2000 cycles are 200 runnings of the loop = 2 millisecond
;   ;adding the rts that is 6 cycles ~ 2 millisecond
; wait_2ms_loop:
;   dex ;2 cycles
;   beq wait_2ms_loop_end ;2cycles to evaluate + 1 or to 2 to take branch
;   jmp wait_2ms_jump_to_wait ;3cycles
...
;   jsr wait_2ms
;   dey ;2cycles
;   bne wait_18ms_outer_loop
;   rts  ;6 cycles

wait_18_5ms:
  ;2 millisecond with a 1Mhz clock is 2000 clock cycles
  ;one operation is 2 clock cycles so 1000 operations is 2 milisecond
  ldy #10 ;the first evaluation is at 8 since i start decreasing
wait_18_5ms_outer_loop:  
  jsr wait_2ms
  dey ;2cycles
  bne wait_18_5ms_outer_loop
  jsr wait_0_5ms
  rts  ;6 cycles
