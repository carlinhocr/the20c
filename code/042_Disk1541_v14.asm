...
;
; Returns: Carry flag = 1 if DATA is LOW (asserted / PB3 = 1)
;          Carry flag = 0 if DATA is HIGH (released / PB3 = 0)
;-----------------------------------------------------------------------
READ_DATA:
            LDA VIA_PORTB       ; Read live byte from VIA Port B
            AND #%00001000      ; Explicitly isolate bit 3 (PB3)
            BNE .data_is_low    ; If it's NOT zero, pin is 1 (Bus is LOW)
            CLC                 ; If it IS zero, pin is 0 (Bus is HIGH) -> Clear Carry
            RTS
.data_is_low:
            SEC                 ; Pin is 1 -> Set Carry
            RTS

;-----------------------------------------------------------------------
; READ_CLK - Read the current state of the CLK line
;
; Returns: Carry flag = 1 if CLK is LOW (asserted / PB4 = 1)
;          Carry flag = 0 if CLK is HIGH (released / PB4 = 0)
;-----------------------------------------------------------------------
READ_CLK:
            LDA VIA_PORTB       ; Read live byte from VIA Port B
            AND #%00010000      ; Explicitly isolate bit 4 (PB4)
            BNE .clk_is_low     ; If it's NOT zero, pin is 1 (Bus is LOW)
            CLC                 ; If it IS zero, pin is 0 (Bus is HIGH) -> Clear Carry
            RTS
.clk_is_low:
            SEC                 ; Pin is 1 -> Set Carry
