...
;               busy window. Fix: keep the TIGHT microsecond poll but enlarge
;               its count (LDY #$40 -> #$00, ~492 ms -> ~2 s), so per-byte and
;               EOI timing is unchanged and only a genuinely busy drive spins;
;               plus status checks after the data-phase LISTEN/secondary so a
;               real failure aborts cleanly.
;
;===============================================================================
;
;   HARDWARE SETUP:
...
; --- Wait for Ready For Data: listener releases DATA (HIGH) ---
            ...
            ; DATA low for ~984 ms here, but the old poll (LDY #$40 => 16384
            ; iters ~= 492 ms) timed out first, so the first two command bytes
            ; of the WRITE data phase (LISTEN $28, DATA-secondary $61) BOTH
            ; timed out and the stream desynced. SD2IEC creates the file
            ; instantly and always answered inside 492 ms - that is why the bug
            ; only showed on real drives. Format dodged it too: its slow work
            ; is after CLOSE, with no command phase landing in a busy window.
            ;
            ; Fix is ONLY a bigger iteration count - this stays a TIGHT poll
            ; (no DELAY here). READ_DATA is tested every ~22 us, so a ready
            ; listener is caught within microseconds and per-byte / EOI timing
            ; is exactly as it was in v19. We just keep spinning up to ~2 s
            ; before declaring the listener dead, which rides out a busy drive
            ; (post-OPEN file creation, and mid-stream buffer flushes on files
            ; larger than one block) at zero cost when the drive is keeping up.
            ;   LDY #$00 => 256 * 256 = 65536 iters ~= 1.97 s @ 1 MHz.
            LDX #$00            ; inner counter (256)
            LDY #$00            ; outer counter (256) -> ~2 s; raise if a near-
                                ;   full disk pushes file creation past ~2 s
.atn_wait_rfd:
            JSR READ_DATA
...
; --- Wait for Ready For Data: listener releases DATA (HIGH) ---
            ; *** v20 FIX *** Same as IEC_SEND_BYTE_ATN: TIGHT poll (no DELAY),
            ; just a bigger iteration count so it rides out a busy drive (file
            ; creation after OPEN, or a mid-stream buffer flush between blocks)
            ; without slowing the normal per-byte / EOI path. A ready listener
            ; is caught within microseconds. Y is the inner counter but is
            ; restored from the stack on every exit (TYA/PHA at entry, PLA/TAY
            ; below), so the caller's loop index survives.
            ;   LDY #$00 => 256 * 256 = 65536 iters ~= 1.97 s @ 1 MHz.
            LDX #$00            ; inner counter (256)
            LDY #$00            ; outer counter (256) -> ~2 s
.send_wait_ready:
            JSR READ_DATA
