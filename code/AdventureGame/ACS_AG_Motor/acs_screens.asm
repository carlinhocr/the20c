; ============================================================
;  ACS Screens — auto-generated by acs_screens_to_asm.py
; ============================================================

screens_index:
  .word screen_pointers_0  ; s1s1
  .word screen_pointers_1  ; s1s2
  .word screen_pointers_2  ; s1s3
  .word screen_pointers_3  ; startScreen
  .word screen_pointers_4  ; endScreens1s1
  .word screen_pointers_5  ; endScreenDefault
  .word screen_pointers_6  ; s1s4
  .word screen_pointers_7  ; s1s5
  .word screen_pointers_8  ; endScreens1s4
  .word screen_pointers_9  ; s2s1
screens_index_record_length:
  .byte 2  ; each screens_index entry is 1 .word (2 bytes)

screens_pointers:
screen_pointers_0:
  .word screen_0_id                ; s1s1 id                [0,1]
  .word screen_0_name              ; s1s1 name              [2,3]
  .word screen_0_action1           ; s1s1 action1           [4,5]
  .word screen_0_action2           ; s1s1 action2           [6,7]
  .word screen_0_action3           ; s1s1 action3           [8,9]
  .word screen_0_action4           ; s1s1 action4           [10,11]
  .word screen_0_description       ; s1s1 description       [12,13]
  .word screen_0_ascii             ; s1s1 ascii             [14,15]
  .word screen_0_flashlight_on     ; s1s1 flashlight_on     [16,17]
  .word screen_0_flashlight_off    ; s1s1 flashlight_off    [18,19]
screen_pointers_1:
  .word screen_1_id                ; s1s2 id                [20,21]
  .word screen_1_name              ; s1s2 name              [22,23]
  .word screen_1_action1           ; s1s2 action1           [24,25]
  .word screen_1_action2           ; s1s2 action2           [26,27]
  .word screen_1_action3           ; s1s2 action3           [28,29]
  .word screen_1_action4           ; s1s2 action4           [30,31]
  .word screen_1_description       ; s1s2 description       [32,33]
  .word screen_1_ascii             ; s1s2 ascii             [34,35]
  .word screen_1_flashlight_on     ; s1s2 flashlight_on     [36,37]
  .word screen_1_flashlight_off    ; s1s2 flashlight_off    [38,39]
screen_pointers_2:
  .word screen_2_id                ; s1s3 id                [40,41]
  .word screen_2_name              ; s1s3 name              [42,43]
  .word screen_2_action1           ; s1s3 action1           [44,45]
  .word screen_2_action2           ; s1s3 action2           [46,47]
  .word screen_2_action3           ; s1s3 action3           [48,49]
  .word screen_2_action4           ; s1s3 action4           [50,51]
  .word screen_2_description       ; s1s3 description       [52,53]
  .word screen_2_ascii             ; s1s3 ascii             [54,55]
  .word screen_2_flashlight_on     ; s1s3 flashlight_on     [56,57]
  .word screen_2_flashlight_off    ; s1s3 flashlight_off    [58,59]
screen_pointers_3:
  .word screen_3_id                ; startScreen id                [60,61]
  .word screen_3_name              ; startScreen name              [62,63]
  .word screen_3_action1           ; startScreen action1           [64,65]
  .word screen_3_action2           ; startScreen action2           [66,67]
  .word screen_3_action3           ; startScreen action3           [68,69]
  .word screen_3_action4           ; startScreen action4           [70,71]
  .word screen_3_description       ; startScreen description       [72,73]
  .word screen_3_ascii             ; startScreen ascii             [74,75]
  .word screen_3_flashlight_on     ; startScreen flashlight_on     [76,77]
  .word screen_3_flashlight_off    ; startScreen flashlight_off    [78,79]
screen_pointers_4:
  .word screen_4_id                ; endScreens1s1 id                [80,81]
  .word screen_4_name              ; endScreens1s1 name              [82,83]
  .word screen_4_action1           ; endScreens1s1 action1           [84,85]
  .word screen_4_action2           ; endScreens1s1 action2           [86,87]
  .word screen_4_action3           ; endScreens1s1 action3           [88,89]
  .word screen_4_action4           ; endScreens1s1 action4           [90,91]
  .word screen_4_description       ; endScreens1s1 description       [92,93]
  .word screen_4_ascii             ; endScreens1s1 ascii             [94,95]
  .word screen_4_flashlight_on     ; endScreens1s1 flashlight_on     [96,97]
  .word screen_4_flashlight_off    ; endScreens1s1 flashlight_off    [98,99]
screen_pointers_5:
  .word screen_5_id                ; endScreenDefault id                [100,101]
  .word screen_5_name              ; endScreenDefault name              [102,103]
  .word screen_5_action1           ; endScreenDefault action1           [104,105]
  .word screen_5_action2           ; endScreenDefault action2           [106,107]
  .word screen_5_action3           ; endScreenDefault action3           [108,109]
  .word screen_5_action4           ; endScreenDefault action4           [110,111]
  .word screen_5_description       ; endScreenDefault description       [112,113]
  .word screen_5_ascii             ; endScreenDefault ascii             [114,115]
  .word screen_5_flashlight_on     ; endScreenDefault flashlight_on     [116,117]
  .word screen_5_flashlight_off    ; endScreenDefault flashlight_off    [118,119]
screen_pointers_6:
  .word screen_6_id                ; s1s4 id                [120,121]
  .word screen_6_name              ; s1s4 name              [122,123]
  .word screen_6_action1           ; s1s4 action1           [124,125]
  .word screen_6_action2           ; s1s4 action2           [126,127]
  .word screen_6_action3           ; s1s4 action3           [128,129]
  .word screen_6_action4           ; s1s4 action4           [130,131]
  .word screen_6_description       ; s1s4 description       [132,133]
  .word screen_6_ascii             ; s1s4 ascii             [134,135]
  .word screen_6_flashlight_on     ; s1s4 flashlight_on     [136,137]
  .word screen_6_flashlight_off    ; s1s4 flashlight_off    [138,139]
screen_pointers_7:
  .word screen_7_id                ; s1s5 id                [140,141]
  .word screen_7_name              ; s1s5 name              [142,143]
  .word screen_7_action1           ; s1s5 action1           [144,145]
  .word screen_7_action2           ; s1s5 action2           [146,147]
  .word screen_7_action3           ; s1s5 action3           [148,149]
  .word screen_7_action4           ; s1s5 action4           [150,151]
  .word screen_7_description       ; s1s5 description       [152,153]
  .word screen_7_ascii             ; s1s5 ascii             [154,155]
  .word screen_7_flashlight_on     ; s1s5 flashlight_on     [156,157]
  .word screen_7_flashlight_off    ; s1s5 flashlight_off    [158,159]
screen_pointers_8:
  .word screen_8_id                ; endScreens1s4 id                [160,161]
  .word screen_8_name              ; endScreens1s4 name              [162,163]
  .word screen_8_action1           ; endScreens1s4 action1           [164,165]
  .word screen_8_action2           ; endScreens1s4 action2           [166,167]
  .word screen_8_action3           ; endScreens1s4 action3           [168,169]
  .word screen_8_action4           ; endScreens1s4 action4           [170,171]
  .word screen_8_description       ; endScreens1s4 description       [172,173]
  .word screen_8_ascii             ; endScreens1s4 ascii             [174,175]
  .word screen_8_flashlight_on     ; endScreens1s4 flashlight_on     [176,177]
  .word screen_8_flashlight_off    ; endScreens1s4 flashlight_off    [178,179]
screen_pointers_9:
  .word screen_9_id                ; s2s1 id                [180,181]
  .word screen_9_name              ; s2s1 name              [182,183]
  .word screen_9_action1           ; s2s1 action1           [184,185]
  .word screen_9_action2           ; s2s1 action2           [186,187]
  .word screen_9_action3           ; s2s1 action3           [188,189]
  .word screen_9_action4           ; s2s1 action4           [190,191]
  .word screen_9_description       ; s2s1 description       [192,193]
  .word screen_9_ascii             ; s2s1 ascii             [194,195]
  .word screen_9_flashlight_on     ; s2s1 flashlight_on     [196,197]
  .word screen_9_flashlight_off    ; s2s1 flashlight_off    [198,199]
screen_action_offset:
  .byte 4  ; (byte of screen_0_action1 in screens_pointers)
screen_description_offset:
  .byte 12  ; (byte of screen_0_description in screens_pointers)
screen_ascii_offset:
  .byte 14  ; (byte of screen_0_ascii in screens_pointers)
screen_flashlight_on_offset:
  .byte 16  ; (byte of screen_0_flashlight_on in screens_pointers)
screen_flashlight_off_offset:
  .byte 18  ; (byte of screen_0_flashlight_off in screens_pointers)
screen_record_length:
  .byte 20  ; (total .word bytes per screen record)

; ── Screen 0: s1s1 ──────────────────────────
screen_0_id:
  .byte 0

screen_0_name:
  .ascii "s1s1"
  .ascii "e"

screen_0_action1:
  .byte 0  ; LEVANTARSE

screen_0_action2:
  .byte 1  ; MIRAR ALREDEDOR

screen_0_action3:
  .byte 2  ; GRITAR POR AYUDA

screen_0_action4:
  .byte 255  ; none

screen_0_description:
  .ascii "Despiertas sobre piedra húmeda."
  .ascii "La caverna se cerro detrás de ti. "
  .ascii "Un derrumbe de rocas antiguas, pesadas, acomodadas como si el colapso hubiera si"
  .ascii "do final, no accidental."
  .ascii "El frío no duele todavía. "
  .ascii "Sientes un constante goteo de agua, cada vez mas cerca de ti."
  .ascii "e"

screen_0_ascii:
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░#########░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░#         ##░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░#             #░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░#                ##░░░░░░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░░######                   #####░░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░░░#     #                   #    #░░░░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░##     #                     #  # ###░░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░░#     #                      # #     #░░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░░░#    #########                #####    #░░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░░#######         #       ########      #   #░░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░░░#                 #######     ##        #   #░░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░░##                  ##           ##        #   #░░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░░░#                     ###           #       ######░░░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░░##                      ##             ##     ###   ##░░░░░░░░░░░░"
  .ascii "░░░░░░░░░░░░░#####                    #                #   ##       ##░░░░░░░░░░"
  .ascii "░░░░░░░░░░░#      #    ####         ###                ###            #░░░░░░░░░"
  .ascii "░░░░░░░░░░#        ## #    #       #########        ###  #            #░░░░░░░░░"
  .ascii "░░░░░░░░░#          ##      #   ###         ##     #   ########       #░░░░░░░░░"
  .ascii "░░░░░░░░░#         #         ###              ##  #####        ##      #░░░░░░░░"
  .ascii "░░░░░░░░░#        #            #                ##              ##    #░░░░░░░░░"
  .ascii "░░░░░░░░░#       #              #               #                ##   #░░░░░░░░░"
  .ascii "░░░░░░░░░░#     #                #             #                  #  #░░░░░░░░░░"
  .ascii "░░░░░░░░░##    #                 #            #                    # #░░░░░░░░░░"
  .ascii "░░░░░░░░##    ##                 ##            #                  #   #░░░░░░░░░"
  .ascii "e"

screen_0_flashlight_on:
  .ascii ""
  .ascii ""
  .ascii "e"

screen_0_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 1: s1s2 ──────────────────────────
screen_1_id:
  .byte 1

screen_1_name:
  .ascii "s1s2"
  .ascii "e"

screen_1_action1:
  .byte 7  ; VEO QUE CAE AGUA

screen_1_action2:
  .byte 255  ; none

screen_1_action3:
  .byte 255  ; none

screen_1_action4:
  .byte 255  ; none

screen_1_description:
  .ascii "Al levantarte, miras a tu alrededor."
  .ascii "Ves que el agua gotea lenta, pero constantemente, inundando la caverna."
  .ascii "Como si midiera el tiempo."
  .ascii "e"

screen_1_ascii:
  .ascii ""
  .ascii "e"

screen_1_flashlight_on:
  .ascii ""
  .ascii "e"

screen_1_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 2: s1s3 ──────────────────────────
screen_2_id:
  .byte 2

screen_2_name:
  .ascii "s1s3"
  .ascii "e"

screen_2_action1:
  .byte 3  ; MIRAR ENTRADA

screen_2_action2:
  .byte 5  ; LINTERNA

screen_2_action3:
  .byte 6  ; EXPLORAR CAMINO

screen_2_action4:
  .byte 255  ; none

screen_2_description:
  .ascii "La caverna se abre en una cámara amplia pero baja. El techo parece aplastarte co"
  .ascii "n su peso, estalactitas afiladas cuelgan peligrosamente del mismo. Las paredes s"
  .ascii "on rugosas, húmedas al tacto, y reflejan una luz apagada que no tiene fuente cla"
  .ascii "ra. El aire es frío y estancado. Huele a roca mojada y a algo más viejo, casi or"
  .ascii "gánico. El suelo tiene marcas irregulares, como pasos interrumpidos en un camino"
  .ascii " que se pierde en la oscuridad"
  .ascii "e"

screen_2_ascii:
  .ascii ""
  .ascii "e"

screen_2_flashlight_on:
  .ascii ""
  .ascii "e"

screen_2_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 3: startScreen ──────────────────────────
screen_3_id:
  .byte 3

screen_3_name:
  .ascii "startScreen"
  .ascii "e"

screen_3_action1:
  .byte 8  ; COMENZAR EL JUEGO

screen_3_action2:
  .byte 255  ; none

screen_3_action3:
  .byte 255  ; none

screen_3_action4:
  .byte 255  ; none

screen_3_description:
  .ascii "Bienvenido a la aventura gráfica más Espectacular del mundo"
  .ascii "e"

screen_3_ascii:
  .ascii ""
  .ascii "e"

screen_3_flashlight_on:
  .ascii ""
  .ascii "e"

screen_3_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 4: endScreens1s1 ──────────────────────────
screen_4_id:
  .byte 4

screen_4_name:
  .ascii "endScreens1s1"
  .ascii "e"

screen_4_action1:
  .byte 9  ; TERMINAR EL JUEGO

screen_4_action2:
  .byte 255  ; none

screen_4_action3:
  .byte 255  ; none

screen_4_action4:
  .byte 255  ; none

screen_4_description:
  .ascii "El constante goteo era hipnotico. Gota, pausa, gota. El agua comenzo a entumecer"
  .ascii " tus extremidades hasta que dejaste de sentirlas. Un eterno sueño te dio la bien"
  .ascii "venida."
  .ascii "e"

screen_4_ascii:
  .ascii ""
  .ascii "e"

screen_4_flashlight_on:
  .ascii ""
  .ascii "e"

screen_4_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 5: endScreenDefault ──────────────────────────
screen_5_id:
  .byte 5

screen_5_name:
  .ascii "endScreenDefault"
  .ascii "e"

screen_5_action1:
  .byte 9  ; TERMINAR EL JUEGO

screen_5_action2:
  .byte 255  ; none

screen_5_action3:
  .byte 255  ; none

screen_5_action4:
  .byte 255  ; none

screen_5_description:
  .ascii "Asi terminas tus dias atrapado en la Caverna"
  .ascii "e"

screen_5_ascii:
  .ascii ""
  .ascii "e"

screen_5_flashlight_on:
  .ascii ""
  .ascii "e"

screen_5_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 6: s1s4 ──────────────────────────
screen_6_id:
  .byte 6

screen_6_name:
  .ascii "s1s4"
  .ascii "e"

screen_6_action1:
  .byte 10  ; EXPLORAR MARCAS

screen_6_action2:
  .byte 11  ; FORZAR ENTRADA

screen_6_action3:
  .byte 12  ; VOLVER A MIRAR ADELANTE

screen_6_action4:
  .byte 255  ; none

screen_6_description:
  .ascii "Entre las rocas hay rendijas por donde entra aire viciado, pero ninguna lo basta"
  .ascii "nte grande para pasar. Hay marcas irregulares, como si alguien hubiera intentado"
  .ascii " moverlas y se hubiera detenido a mitad de camino. Tal vez no sea una opcion vol"
  .ascii "ver por ahí, pero la idea de explorar una caverna a oscuras no parece tentadora."
  .ascii "e"

screen_6_ascii:
  .ascii ""
  .ascii "e"

screen_6_flashlight_on:
  .ascii ""
  .ascii "e"

screen_6_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 7: s1s5 ──────────────────────────
screen_7_id:
  .byte 7

screen_7_name:
  .ascii "s1s5"
  .ascii "e"

screen_7_action1:
  .byte 12  ; VOLVER A MIRAR ADELANTE

screen_7_action2:
  .byte 255  ; none

screen_7_action3:
  .byte 255  ; none

screen_7_action4:
  .byte 255  ; none

screen_7_description:
  .ascii "Este es el PITFALL"
  .ascii "e"

screen_7_ascii:
  .ascii ""
  .ascii "e"

screen_7_flashlight_on:
  .ascii ""
  .ascii "e"

screen_7_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 8: endScreens1s4 ──────────────────────────
screen_8_id:
  .byte 8

screen_8_name:
  .ascii "endScreens1s4"
  .ascii "e"

screen_8_action1:
  .byte 9  ; TERMINAR EL JUEGO

screen_8_action2:
  .byte 255  ; none

screen_8_action3:
  .byte 255  ; none

screen_8_action4:
  .byte 255  ; none

screen_8_description:
  .ascii "El suelo comienza a temblar y entrada empieza a desmoronarse sobre ti. Lo ultimo"
  .ascii " que piensas es que no estaban sellando la salida. Estaban sosteniendo el resto"
  .ascii "Haz muerto"
  .ascii "e"

screen_8_ascii:
  .ascii ""
  .ascii "e"

screen_8_flashlight_on:
  .ascii ""
  .ascii "e"

screen_8_flashlight_off:
  .ascii ""
  .ascii "e"

; ── Screen 9: s2s1 ──────────────────────────
screen_9_id:
  .byte 9

screen_9_name:
  .ascii "s2s1"
  .ascii "e"

screen_9_action1:
  .byte 255  ; none

screen_9_action2:
  .byte 255  ; none

screen_9_action3:
  .byte 255  ; none

screen_9_action4:
  .byte 255  ; none

screen_9_description:
  .ascii ""
  .ascii "e"

screen_9_ascii:
  .ascii ""
  .ascii "e"

screen_9_flashlight_on:
  .ascii "Luego de explorar el pasillo, te encuentras con una sala grande. Dos tuneles se "
  .ascii "abren frente a ti. Decidir en estas condiciones es dificil, pero necesario. El a"
  .ascii "gua sigue subiendo lentamente."
  .ascii "e"

screen_9_flashlight_off:
  .ascii "A ciegas tanteas la pared del pasillo, hasta que se acaba. Tienes la sensacion d"
  .ascii "e que llegaste a un lugar mas espacioso. Recorres la pared a tientas y encuentra"
  .ascii "s dos grandes huecos. ¿Tal vez sean tuneles?. No te detienes demasiado a pensarl"
  .ascii "o, sientes mas agua en contacto con tu cuerpo."
  .ascii "e"

; ── Total screen count ──────────────────────────────────────
screen_count:
  .byte 10
