...
def to_asm(puzzles, action_ids, screen_ids):
    lines = []
    lines.append("; ============================================================")
    ...
    for index, (key, puz) in enumerate(puzzles.items()):
        ...
        lines.append(f"  .word {label}_screen_name          ; {name} screen_name          [{offset},{offset+1}]") ; offset += 2
        lines.append(f"  .word {label}_screen_id            ; {name} screen_id            [{offset},{offset+1}]") ; _scr = offset ; offset += 2
    ...
    lines.append(f"puzzle_screen_offset:")
    lines.append(f"  .byte {puz0_screen_offset}  ; (byte of puzzle_0_screen_id in puzzles_pointers)")
    ...
    for index, (key, puz) in enumerate(puzzles.items()):
        ...
        scr_name  = puz.get("Screen",   "").replace('"', '\\"')
        scr_id    = puz.get("ScreenID", "").strip()
        try:
            scr_id_byte = int(scr_id)
        except (ValueError, TypeError):
            scr_id_byte = 255  # sentinel: no screen assigned
        ...
        # Screen name
        lines.append(f"{label}_screen_name:")
        lines.append(f'  .ascii "{scr_name}"')
        lines.append('  .ascii "e"')
        lines.append("")

        # Screen ID
        lines.append(f"{label}_screen_id:")
        lines.append(f"  .byte {scr_id_byte}  ; {scr_name if scr_name else 'none'}")
        lines.append("")
...
def main():
    ...
    screen_ids = load_name_to_id(SCREENS_FILE, id_field="ID",   name_field="Name")
    ...
    asm = to_asm(puzzles, action_ids, screen_ids)
