...
def ascii_lines(text):
    ...
    Newlines are kept inside the quoted strings as \\n.
    """
    segments = text.split("\n")
    out = []
    for si, seg in enumerate(segments):
        # Append \n to rejoin with next segment (except the last)
...
def emit_token_dictionary_asm(id_to_token):
    ...
    # Token data: each token as .ascii, \n kept inside the string
    for tid in sorted(id_to_token.keys()):
        tok = id_to_token[tid]
        tok_repr = tok.replace('\n', '\\n')
        tok_asm = tok.replace('"', '\\"').replace('\n', '\\n')
        
        lines.append(f"token_{tid}:  ; \"{tok_repr}\"")
        lines.append(f'  .ascii "{tok_asm}"')
        lines.append("")
...
def to_asm(screens, action_ids):
    ...
    for index, (key, scr) in enumerate(screens.items()):
        ...
        # Description (original — commented out, replaced by compressed)
        ...
        # FlashlightOn (original — commented out, replaced by compressed)
        ...
        # FlashlightOff (original — commented out, replaced by compressed)
